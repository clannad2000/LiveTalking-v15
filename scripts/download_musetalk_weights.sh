...
hf download TMElyralab/MuseTalk --local-dir "$MODEL_DIR"


echo "Downloading SD VAE weights to $MODEL_DIR/sd-vae-ft-mse..."
hf download stabilityai/sd-vae-ft-mse --local-dir "$MODEL_DIR/sd-vae-ft-mse"


# echo "Downloading Whisper weights to $MODEL_DIR/whisper..."
# hf download openai/whisper-tiny --local-dir "$MODEL_DIR/whisper" --local-dir-use-symlinks False --include "config.json" "pytorch_model.bin" "preprocessor_config.json"

# echo "Downloading DWPose weights to $MODEL_DIR/dwpose..."
# hf download yzd-v/DWPose --local-dir "$MODEL_DIR/dwpose" --local-dir-use-symlinks False --include "dw-ll_ucoco_384.pth"

# # Download SyncNet weights to syncnet subdirectory
# echo "Downloading SyncNet weights to $MODEL_DIR/syncnet..."
# hf download ByteDance/LatentSync --local-dir "$MODEL_DIR/syncnet" --local-dir-use-symlinks False --include "latentsync_syncnet.pt"


echo "Downloading Face Parse Bisent weights to $MODEL_DIR/face-parse-bisent..."
hf download ManyOtherFunctions/face-parse-bisent --local-dir "$MODEL_DIR/face-parse-bisent" 79999_iter.pth resnet18-5c106cde.pth
